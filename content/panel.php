...
$slUser = mysqli_query($TIG, "SELECT * FROM ver_duvida.users
								JOIN ver_duvida.usu_verifica ON (users.id = usu_verifica.fk_id_usu)
								WHERE users.id = '".$_GET['user']."' ") or die( mysqli_error( $TIG ) );
								
					$row_User = mysqli_fetch_assoc($slUser);
$slInteresse = mysqli_query($TIG, "SELECT * FROM ver_duvida.users
								JOIN ver_duvida.usu_verifica ON (users.id = usu_verifica.fk_id_usu)
								LEFT JOIN ver_duvida.materia_verifica ON (usu_verifica.id_materia = materia_verifica.pk_materia_id)
