<?php
 ...
 /* Instrução de consulta para paginação com MySQL */  
	 ...
	 $TIG = new PDO("mysql:host=localhost; dbname=ver_duvida;", "5TIG", "testehost", $opcoes); 
 
	$RsPerguntas =  "SELECT * FROM ver_duvida.pergunta_verifica
						JOIN ver_duvida.materia_verifica
							ON (pergunta_verifica.fk_id_materia = materia_verifica.pk_materia_id)
						LEFT JOIN ver_duvida.resposta_verifica
							ON (pergunta_verifica.pk_id_pergunta = resposta_verifica.fk_id_pergunta)
						LEFT JOIN ver_duvida.users
 ...
 $sqlContador = "SELECT COUNT(*) AS total_registros FROM ver_duvida.pergunta_verifica";   
    ...
    <?php if (!empty($dados)):
		 ...
		 foreach ($dados as $row_RsPerguntas): 
            ...
            <?php if (isLoggedIn()){ ?>
				...
				<?php } 	
                    $RsQtdResposta =  "SELECT COUNT(*) AS qtd_respostas FROM ver_duvida.resposta_verifica WHERE (resposta_verifica.fk_id_pergunta = '".$row_RsPerguntas->pk_id_pergunta."')";  
            		 if ((int)$nivel > 0){ ?>
