...
if(isset($_GET['pagina']) != ""){
	...
	include(SQL_PATH."pag_comentarioUser.php");?>

<div id="userComentarios" style="display: <?php echo $show; ?>">
	<hr />    
	<h2>Perguntas Comentadas &#124; <small><?php echo $_SESSION["user_name"]; ?></small></h2>
	...
	<?php if (!empty($dados)):
	 ...
	 <div class='box-paginacao'>     
		   <a class='box-navegacao <?=$exibir_botao_inicio?>' href="index.php?p=perfil&user=<?php echo $user; ?>&pag=<?=$primeira_pagina?>" title="Primeira Página">&#124; Primeira  &#124; </a> 
		   <a class='box-navegacao <?=$exibir_botao_inicio?>' href="index.php?p=perfil&user=<?php echo $user; ?>&pag=<?=$pagina_anterior?>" title="Página Anterior">Anterior  &#124; </a> 
		<?php
	  /* Loop para montar a páginação central com os números */   
	  for ($i=$range_inicial; $i <= $range_final; $i++):   
		$destaque = ($i == $pagina_atual) ? 'destaque' : '' ;  ?>   
		<a class='box-numero <?=$destaque?>' href="index.php?p=perfil&user=<?php echo $user; ?>&pag=<?=$i?>"><?=$i?> </a>   
	  <?php endfor; ?>    
		<a class='box-navegacao <?=$exibir_botao_final?>' href="index.php?p=perfil&user=<?php echo $user; ?>&pag=<?=$proxima_pagina?>" title="Próxima Página">&#124; Pr&oacute;xima  &#124; </a>  
		<a class='box-navegacao <?=$exibir_botao_final?>' href="index.php?p=perfil&user=<?php echo $user; ?>&pag=<?=$ultima_pagina?>" title="Última Página">&Uacute;ltimo  &#124; </a> 
	 </div>   
	<?php else: ?>   
